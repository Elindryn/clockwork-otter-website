...
**Client-side JavaScript (decided 2026-08-10, Phase 7).** This site ships zero client-side JavaScript by default, per CLAUDE.md's Technology Stack section ("minimal JavaScript — reach for it only where static HTML/CSS cannot do the job"). Phase 7's accessible mobile navigation toggle is the **first and, as of this decision, only exception**: a small inline vanilla-JS script in `Navigation.astro` toggling `aria-expanded` and a visibility class on the mobile menu — genuinely necessary because `aria-expanded` is a live state attribute that CSS-only techniques (e.g. a checkbox hack) can't drive correctly, and a real `<button>` with real ARIA semantics was judged the correct choice over a checkbox/label pattern masquerading as one. It ships with a `<noscript>` fallback that keeps the menu links reachable even if JavaScript fails to load. Future phases should keep this the exception, not a precedent to build on casually — reach for static HTML/CSS first, and if JS is genuinely required again, match this same standard (small, inline, progressively enhanced, with a documented reason).
