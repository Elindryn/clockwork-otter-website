...
**Legal pages (decided 2026-08-07, Phase 1 review).** Privacy Policy and Terms of Service are real, permanent pages linked from the footer — not deferred. Content must stay clearly-marked placeholder (e.g. "Placeholder — Privacy Policy content pending legal review") until real legal copy is supplied. Never generate actual legal/policy text.

**Font loading (decided 2026-08-07, Phase 1 review).** Google Fonts CDN loading continues through development. Self-hosting will be evaluated during Phase 6 (Polish). Keep the font-loading mechanism centralized (one place declaring the `<link>`/`@font-face` rules) so that switching to self-hosted fonts later touches one file, not every component.
